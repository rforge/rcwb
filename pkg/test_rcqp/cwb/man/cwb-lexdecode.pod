...
This utility can be used to print all or part of the lexicon (or part of it) of a 
positional attribute to standard output. Optionally, frequency information can also
be printed.
...
Alternatively, a regular expression can be specified (with B<-p>); in this case
only entries that match that regular expression will be printed out. This
is a normal CWB regular expression, and so all the usual rules for string-level
regular expressions in CQP apply: the PCRE syntax for regular expressions is used,
the regexp matches the whole item, and it is case-sensitive and accent-sensitive by default. 
Case-sensitivity and accent-sensitivity can be turned off with the B<-c> and B<-d> flags respectively.

Command-line B<options> must be specified before the I<corpus> name. The corpus name is compulsory, and
must be given in the ALL-UPPERCASE version.

B<cwb-lexdecode> will look for the specified corpus in the registry directory, as usual. If a registry
...
Activates file-input mode. The specified I<file> is opened and read, one line at a time
(use C<-> to read from standard input). Each line is treated as a single word/annotation,
and the corresponding lexicon entry is printed. Note that only precise, character-for-character 
matches are found; no regexp matching is used. Note that the B<-N> option changes this 
behaviour significantly: numbers are expected in the input file instead.
...
L<cwb-itoa>, L<cwb-makeall>, L<cwb-scan-corpus>, L<cwb-s-decode>, L<cwb-s-encode>.
