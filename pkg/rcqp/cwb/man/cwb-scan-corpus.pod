...
Each bucket can hold a single tuple of key values.  If there are more different types (i.e. key value combinations) than buckets,
they have to be stored in overflow chains, reducing the computational efficiency of the hashing algorithm.
...
do not need to understand this option.  The default setting of 1,000,000 buckets will be sufficient for most applications.

=item B<-C>        

Enables cleanup of the output data. "Cleanup" means that any token values that are not "regular" words will be omitted.
A regular word consists only of letters, numbers and hyphens and does not end with a hyphen.
...
L<cwb-itoa>, L<cwb-lexdecode>, L<cwb-makeall>, L<cwb-s-decode>, L<cwb-s-encode>.
