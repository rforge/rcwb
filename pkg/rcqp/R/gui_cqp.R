...
## The  frame (main frame holding a table)
kwic.frame <- tkframe(dlg)

## for the first time print an empty table

xscr <- tkscrollbar(kwic.frame, repeatinterval=5,orient="horizontal",
                       command=function(...)tkxview(txt,...))
yscr <- tkscrollbar(kwic.frame, repeatinterval=5,
                       command=function(...)tkyview(txt,...))
txt <- tktext(kwic.frame,bg="white",font="courier",
...
#   xscr <-tkscrollbar(kwic.frame,orient="vertical", repeatinterval=5, command=function(...)tkyview(table.kwic,...))
#   table.kwic <- tkwidget(kwic.frame,"table",rows=nr,cols=nc,titlerows=0,titlecols=0,
#                      height=3,width=5,
#                      yscrollcommand=function(...) tkset(xscr,...))
# 
...
#   tkpack(table.kwic, side="left", fill="y", expand=TRUE);
# #   tkpack(xscr);
#   tkpack(xscr, side="left", fill="y");
#   
...
get.tclArray <- function(kwic.matrix) {
  tclArray <- tclArray()
