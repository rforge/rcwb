...
rcqp is a wrapper onto the CWB software. The CWB, used in the field of corpus
linguistics, allows for index and querying large annotated corpora. The rcqp
wrapper turn cwb into a shared library and allow using R for running cwb and
turning results into statistical analyses.
