...
\texttt{subcorpus}\index{subcorpus}, \texttt{cqp\_flist} and \texttt{cqp\_ftable} objects (see below).
...
k <- cqp_kwic(sc, sort.anchor="target", sort.anchor.offset=0, sort.anchor.attribute="pos", right.context=10, left.context=10)
...
fl <- cqp_flist(c, "word", cutoff=0.01);
summary(fl);
fl[1:30]
#
# get only the forms with freq > 100
fl <- cqp_flist(c, "word", cutoff=100);
summary(fl);
fl[1:30]
@
...
fl <- cqp_flist(sc, c("match", "matchend"), "word", left.context=5, right.context=5);
summary(fl);
fl[1:10]
@
