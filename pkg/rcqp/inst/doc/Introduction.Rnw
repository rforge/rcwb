...
\begin{itemize}
\item You can call CWB through the "cqi\_*" set of functions. These 
...
All functions are prefixed with cqi\_.
...
In order to get information on the data model used into CWB, see section 1.2 
...
Each column represent an \textbf{attribute}.

The first n columns represent "\textbf{structural attribute}": this type of 
attribute define spans of consecutive tokens (like XML tag surrounding 
tokens), called regions. In each structural attribute, each region
as an id, called \textbf{struc}. While the id is the 
...
instance, the np\_h structural attribute, giving the head of the noun 
