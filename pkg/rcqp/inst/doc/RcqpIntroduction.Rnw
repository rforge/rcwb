...
sc
#
# if you want to change the lines printed:
print(sc, from=2, to=5);
#
# if you want more access on the kwic presentation
# (sorting, printing), you can construct a kwic object):
k <- kwic.cqp_subcorpus(sc, sort.anchor="target", sort.anchor.attribute="pos")
print(k, from=5, to=10)
@

An object \texttt{cqp\_subcorpus}
...
An object \texttt{cqp\_flist} hold a frequency list and is created with the function 
\texttt{flist()}.
...
c <- corpus("DICKENS");
fl <- flist.cqp_corpus(c, "word");
summary(fl);
#
# get only the 1% most frequent forms
fl <- flist.cqp_corpus(c, "word", cutoff=0.01);
summary(fl);
fl[1:30]
#
# get only the forms with freq > 100
fl <- flist.cqp_corpus(c, "word", cutoff=100);
summary(fl);
fl[1:30]
@
...
fl <- flist.cqp_subcorpus(sc, "target", "word");
summary(fl);
#
fl <- flist.cqp_subcorpus(sc, "target", "pos");
fl;
#
# or with the match..matchend span, extended to -5 .. +5 tokens :
fl <- flist.cqp_subcorpus(sc, c("match", "matchend"), "word", left.context=5, right.context=5);
summary(fl);
fl[1:10]
@
...
An object \texttt{cqp\_ftable} hold a frequency table: a cross-tabulated
frequency count according to two attributes.

This object is created with the functions 
\texttt{ftable.cqp\_corpus()} or
\texttt{ftable.cqp\_subcorpus()}.

\paragraph{ftable.cqp\_corpus}

\texttt{cqp\_ftable.cqp\_corpus} allows for creating frequency tables using a 
...
library(reshape);
c <- corpus("DICKENS");
f <- ftable.cqp_corpus(c, "novel_title", "pos");
f[1:10,]
#
...
use two structural attributes as argument in \texttt{cqp\_ftable.cqp\_corpus},
...
f <- ftable.cqp_corpus(c, "novel_title", "s")
f[1:10,]
@
...
f <- ftable.cqp_corpus(c, "novel_title", "pp_h", structural.attribute.unique.id=TRUE)
f[1:10,]
@
...
f <- ftable.cqp_corpus(c, "novel_title", "pp_h")
f[1:10,]
@
...
The \texttt{ftable.cqp\_subcorpus} function is mainly a wrapper on the 
\texttt{cqi\_fdist2} function. It retuns a data.frame with strings rather 
than a matrix with ids like \texttt{cqi\_fdist2}.

<<ftable_session5, echo=TRUE>>=
c <- corpus("DICKENS");
sc <- subcorpus(c, "\"from\" @ [] \"to\" []")
f <- ftable.cqp_subcorpus(sc, "target", "word", "matchend", "word");
f[1:10,]
@
