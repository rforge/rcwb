...
Get a cross-tabulated table of the strings of a given positional 
attribute in a subcorpus\index{subcorpus} * strings of a given.
...
corpus\index{corpus} object. A \texttt{corpus} object is created with the function \texttt{corpus()}.
This object may be used with the two functions below, as well as for creating 
\texttt{subcorpus}\index{subcorpus}, \texttt{flist} and \texttt{ftable} objects (see below).
...
Get a kwic summary of the size and content of the subcorpus\index{subcorpus}.
...
\subsubsection{Create a frequency table}
...
\texttt{ftable} may be applied either on a corpus\index{corpus}, or a subcorpus\index{subcorpus}.
...
<<ftable_session1a, fig=FALSE, echo=TRUE, eval=FALSE>>=
library(reshape);
c <- corpus("DICKENS");
...
<<ftable_session1b, fig=FALSE, echo=TRUE, eval=FALSE>>=
#
...
use two structural attributes\index{attribute!structural} as arguments in \texttt{ftable},
...
<<ftable_session2, echo=TRUE, eval=FALSE>>=
...
<<ftable_session3, echo=TRUE, eval=FALSE>>=
...
<<ftable_session4, echo=TRUE, eval=FALSE>>=
...
Applied on a subcorpus\index{subcorpus}, the \texttt{ftable} function is mainly a wrapper on the 
