    ...
    if (sscanf(config[config_pointer], "%s", command) > 0) {
      if(command[0] == '-') {
          ...
          if(sscanf(config[config_pointer],"%2s:%d:%s %s",command,&weight,filename,dummy)!=3) {
            ...
            while((nw = fscanf(wordlist,"%s %s",word1,word2))>0) {
              ...
              if (! (cl_string_validate_encoding(word1, charset, 0)
                ...
                Rprintf( "ERROR: character encoding error in the word-list input file with the input word list.\n");
                Rprintf( "       (The error occurs on line %d.)\n", nl);
                rcqp_receive_error(1);
              }
              if (nw != 2)
                Rprintf("WARNING: Line %d in word list '%s' contains %d words, ignored.\n",nl,filename,nw);
              else {
        ...
        case 'C': 
          ...
          break;
        default:
          Rprintf("ERROR: unknown feature: %s\n",config[config_pointer]);
          rcqp_receive_error(1);
          break;
        }
      }
