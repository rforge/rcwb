...
B<cwb-align-encode> reads its input from a C<.align> file. The C<.align> file format uses CWB-internal
corpus positions to identify aligned regions of the corpus.  It is therefore
recommended not to generate such input files manually. Rather, it is better
...
utility from the CWB/Perl interface. This utiltiy imports external
...
=for comment delete the above if it turns out reverse mode really has been removed. 
